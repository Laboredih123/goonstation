	...
	*/
	read_map(dmm_text as text, coordX as num, coordY as num, coordZ as num, tag as text, flags as num)
		...
		for(var/posZ = 1 to length(gridLevels))
			...
			for(var/posY in length(yReversed) to 1 step -1)
			...
			if(flags && posZ == 1) // do this only once so we don't delete our own stuff if it's big!!!
				for(var/internalPosZ in 1 to length(gridLevels))
					...
					for(var/posY in 1 to length(yLines))
						var yLine = yLines[posY]
						for(var/posX in 1 to length(yLine)/key_len)
...
//-- Supplemental Methods ------------------------------------------------------
	...
	proc
		parse_grid(models as text, xcrd, ycrd, zcrd)
			...
			var/list/atomModels = splittext(models, comma_delim)
			//First pass: Load areas
			for(var/atomModel in atomModels)
