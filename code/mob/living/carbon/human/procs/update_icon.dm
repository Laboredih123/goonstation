...
/mob/living/carbon/human/proc/update_id(head_offset)
	if (src.wear_id)
		...
		if (wear_state in typeinfo?.clothing_icon_states["id"])
			wear_id.icon = typeinfo.clothing_icons["id"]
			no_offset = 1
		else
...
/mob/living/carbon/human/proc/update_suit()
	...
	if (src.wear_suit)
		...
		if (src.wear_suit.worn_material_texture_image)
		...
		src.AddOverlays(src.tail_standing, "tail", 1) // i blame pali for giving me this power
		src.AddOverlays(src.tail_standing_oversuit, "tail_oversuit", 1)
		src.AddOverlays(src.detail_standing_oversuit, "detail_oversuit", 1)
...
/mob/living/carbon/human/proc/update_back(body_offset)
	if (src.back)
		...
		if (src.back.worn_material_texture_image)
		...
		src.ClearSpecificOverlays("wear_back", "material_bacl")
...
/mob/living/carbon/human/proc/update_glasses(head_offset)
	if (src.glasses)
		...
		if (src.glasses.worn_material_texture_image)
...
/mob/living/carbon/human/proc/update_ears(head_offset)
	if (src.ears)
		...
		if (src.ears.worn_material_texture_image)
...
/mob/living/carbon/human/proc/update_mask(head_offset)
	...
	if (src.wear_mask)
		...
		if (src.wear_mask.worn_material_texture_image)
...
/mob/living/carbon/human/proc/update_head(head_offset)
	...
	if (src.head)
		...
		if (src.head.worn_material_texture_image)
...
/mob/living/carbon/human/proc/update_belt(body_offset)
	if (src.belt)
		...
		if (src.belt.worn_material_texture_image)
			...
			src.ClearSpecificOverlays(null, "material_belt")
...
/mob/living/carbon/human/update_face()
	...
	if (src?.organHolder?.head)
		...
		if(my_head.head_image_nose)
			AddOverlays(my_head.head_image_nose, "nose", 1)
		else
			ClearSpecificOverlays("nose", retain_cache=1)
		...
		if (src.image_eyes_L && src.image_eyes_L.icon_state != "none"&& src.organHolder?.left_eye)
			...
			AddOverlays(image_eyes_L, "eyes_L", 1, 1)
		else
			ClearSpecificOverlays("eyes_L", retain_cache=1)
		...
		if (src.image_eyes_R && src.image_eyes_R.icon_state != "none" && src.organHolder?.right_eye)
			...
			AddOverlays(image_eyes_R, "eyes_R", 1)
		else
			ClearSpecificOverlays("eyes_R", retain_cache=1)
		...
		if(!seal_hair)
			if (AHH.mob_appearance_flags & HAS_HUMAN_HAIR || src.hair_override)
				if(image_cust_one?.icon_state && image_cust_one.icon_state != "none")
					AddOverlays(image_cust_one, "hair_one", 1)
				else
					ClearSpecificOverlays("hair_one", retain_cache=1)

				if(image_cust_two?.icon_state && image_cust_two.icon_state != "none")
					AddOverlays(image_cust_two, "hair_two", 1)
				else
					ClearSpecificOverlays("hair_two", retain_cache=1)

				if(image_cust_three?.icon_state && image_cust_three.icon_state != "none")
					AddOverlays(image_cust_three, "hair_three", 1)
				else
					ClearSpecificOverlays("hair_three", retain_cache=1)
			else
				ClearSpecificOverlays(1, "hair_one", "hair_two", "hair_three")
			...
			if (AHH.mob_appearance_flags & HAS_SPECIAL_HAIR || src.special_hair_override)
				if(image_special_one?.icon_state && image_special_one.icon_state != "none")
					AddOverlays(image_special_one, "hair_special_one", 1)
				else
					ClearSpecificOverlays("hair_special_one", retain_cache=1)

				if(image_special_two?.icon_state && image_special_two.icon_state != "none")
					AddOverlays(image_special_two, "hair_special_two", 1)
				else
					ClearSpecificOverlays("hair_special_two", retain_cache=1)

				if(image_special_three?.icon_state && image_special_three.icon_state != "none")
					AddOverlays(image_special_three, "hair_special_three", 1)
				else
					ClearSpecificOverlays("hair_special_three", retain_cache=1)
			else
...
/mob/living/carbon/human/update_body(force = FALSE)
	...
	src.AddOverlays(src.body_standing, "body", 1)
	src.AddOverlays(src.hands_standing, "hands", 1)
	src.AddOverlays(src.tail_standing, "tail", 1) // i blame pali for giving me this power
	src.AddOverlays(src.tail_standing_oversuit, "tail_oversuit", 1, 1)
	src.AddOverlays(src.detail_standing_oversuit, "detail_oversuit", 1, 1)
...
/mob/living/carbon/human/UpdateDamageIcon()
	...
	if (ismonkey(src))
		...
		if(burn_state || brute_state)
			...
			ClearSpecificOverlays("body_damage", retain_cache=1)
	else
		...
		if(burn_state || brute_state)
			#define UPDATE_OVERLAY(X) \
				if(src.X ## _standing?.icon_state && src.X ## _standing.icon_state != "00") {\
					...
					src.ClearSpecificOverlays(#X, retain_cache=1); \
				}
			...
			ClearSpecificOverlays(1, "body_damage", "head_damage", "l_arm_damage", "r_arm_damage", "l_leg_damage", "r_leg_damage")
