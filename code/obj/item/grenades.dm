...
/obj/item/pipebomb/frame
	...
	attackby(obj/item/W, mob/user)
		if(iswrenchingtool(W) && state < 2) // buildable atmos pipes using "pipe frames"
			var/setting_context_actions = list()
			for(var/actionType in childrentypesof(/datum/contextAction/atmos_pipes)) //see context_actions.dm for those
				var/datum/contextAction/atmos_pipes/action = new actionType(user)
				setting_context_actions += action
			user.showContextActions(setting_context_actions, src, new /datum/contextLayout/experimentalcircle)
