...
/obj/machinery/filter_control
	name = "filter control"
	icon = 'icons/obj/atmospherics/filter_control.dmi'
...
/obj/machinery/filter_control/New()
	..()
	SPAWN(5 DECI SECONDS)	//wait for world
		for(var/obj/machinery/atmospherics/unary/inlet_filter/F as anything in machine_registry[MACHINES_INLETS])
			if(F.id == src.id)
				F.f_mask = src.f_mask
